...
$comando="SELECT codigo_pagamento, nome, sobrenome, valor_total, DATE_FORMAT(data_cadastro, '%d/%m/%Y') AS data from ordem_pagamentos WHERE data_cadastro BETWEEN STR_TO_DATE('$data_inicial', '%Y-%m-%d') AND STR_TO_DATE('$data_final', '%Y-%m-%d') AND tipo='devolucao'";
    ...
    $html.="<h1 style='padding: 0px; margin: 0px; text-align: center; font-family: Helvetica Neue,Helvetica,Arial,sans-serif;'>Ordens de Devoluções</h1>";  
    ...
    $html.="<TR >";
    $html.="<th style='font-family: Helvetica Neue,Helvetica,Arial,sans-serif; padding: 10px 20px; border: 1px solid #DDD; font-weight: bold; font-size: 14px; text-align: center;'>DEVOLUÇÃO</th>";
...
$dompdf->render();
$dompdf->stream("rel_devolucoes_$hoje.pdf");
?>